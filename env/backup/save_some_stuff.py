...
class slidingMegaStockTestEnv(gym.Env):
    ...
    def __init__(self, start_day, end_day, trading_days, ticker_list, day=0, money=10000, algoMode=None):
        ...
        self.iteration = 0
        self.action_space = spaces.Box(low=-50, high=50, shape=(len(ticker_list),), dtype=np.int8)
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            if begin_total_asset >= self.init_budget - 900:
                ...
                self.stop_loss = True
                self.state = [begin_total_asset] + self.data[self.ticker_list].iloc[0].tolist() + [0 for _ in range(len(self.ticker_list))]
                self.reward = 0
                self.asset_memory.append(self.state[0])
                self.day += 1
                self.data = self.test_daily_data[self.day]
